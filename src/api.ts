...
app.post('/api/analyze-competitor-set', async (req: Request, res: Response) => {
  try {
    ...
    // Normalize isSubject from string to boolean (n8n sends "true"/"false" strings)
    for (const comp of competitors) {
      if (typeof comp.isSubject === 'string') {
        comp.isSubject = comp.isSubject.toLowerCase() === 'true';
      }
    }
    ...
    // Validate exactly 1 subject and 1-10 competitors
    ...
    if (competitorCount < 1 || competitorCount > 10) {
      return res.status(400).json({
        error: `Between 1 and 10 competitors required (got ${competitorCount})`
      });
    }

    console.log(`📊 Starting Facebook Ads competitive analysis via API (1 subject + ${competitorCount} competitors)...`);
    ...
    // Soft no-ads result: return 200 so n8n can branch on status instead of catching an error
    if (result.status === 'no_ads_found' || !result.subject || !result.gaps) {
      return res.json({
        success: false,
        status: 'no_ads_found',
        analysis_id: null,
        subject_brand_name: result.subject_brand_name,
        message: result.message || 'No active ads found for subject brand in Meta Ad Library',
        subject: null,
        gaps: null,
        recommendations: [],
...
app.post('/api/analyze-google-competitor-set', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`📊 Starting Google Ads competitive analysis via API (1 subject + ${competitorCount} competitors)...`);
    ...
    if (result.status === 'no_ads_found' || !result.subject || !result.gaps) {
      return res.json({
        ...
        platform: 'google',
        subject_brand_name: result.subject_brand_name,
        message: result.message || 'No active ads found for subject brand in Google Ads Transparency Center',
        subject: null,
        gaps: null,
        recommendations: [],
        market_insights: null,
