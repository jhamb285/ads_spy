/**
 * Populate Google Sheets with all default brands from config.ts - BATCH VERSION
 *
 * This script uses the batch update API to write all brands in ONE API call,
 * bypassing the 60 writes/minute limit.
 *
 * Usage:
 *   npm run populate-sheets-batch
 */
...
async function main() {
  console.log('📊 Populating Google Sheets with all 89 brands using BATCH API...\\n');
  ...
  if (!brandsSheet) {
    ...
    if (rows.length > 0) {
      ...
      await brandsSheet.clear('A2:G1000'); // Clear all data rows
    }
  }

  // Prepare rows for batch write (ONE API call for all 89 brands!)
  console.log(`📝 Writing all ${brands.length} brands in ONE batch operation...\\n`);
  ...
  // Use batch update API - bypasses the 60 writes/minute limit!
  await brandsSheet.addRows(rows);

  console.log(`\\n✅ Successfully added all ${brands.length} brands in ONE API call!\\n`);
