...
export async function syncBrandsFromSheet(writeBack: boolean = false): Promise<void> {
  ...
  try {
    ...
    // Clean up private key (remove quotes, trailing characters)
    ...
    for (const brand of validBrands) {
      try {
        ...
        const existing = await pool.query(
          'SELECT id, active, avatar, brand_name, min_active_days FROM adspy_brands WHERE page_url = $1',
          [brand.pageUrl]
        );
    ...
    if ((deactivateResult.rowCount ?? 0) > 0) {
      ...
      deactivateResult.rows.forEach((row: any) => {
    ...
    if (writeBack) {
      ...
      const statsMap = new Map<string, { lastScraped: Date | null, totalAds: number }>();
